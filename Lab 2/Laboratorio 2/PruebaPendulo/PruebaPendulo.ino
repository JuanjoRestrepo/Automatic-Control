...
float xhat_new[4]={0,0,0,0};
float A[4][4]={{0,0,1,0},{0,0,0,1},{0,-28.1714,-3.21283,0.928499},{0,86.1096,-3.17547,-2.83808}};
float B[4]={0,0,9.39423,9.28499};
//float kgain[4]={-0.358709, 26.1799, -0.727478, 2.23836};//Hallados por asignación de polos
float L[4][2]={{8.08382, 1.30548}, {-2.80751, 11.8653}, {2.80929, -19.9412},{-14.8373, 101.942}};



//INSERTAR SUS GANANCIAS LQR EN LA SGTE. LÍNEA 
float kgain[4]={-1.0541, 84.6646, -1.9565, 12.6891};// 
...
void loop() {
  ...
  if (currentMicros - previousMicros >= sampleTime) {
    ...
    /*Turn on motor*/
    //float u=0;
    xhat_new[0]=xhat[0]+0.001*(A[0][0]*xhat[0]+A[0][1]*xhat[1]+A[0][2]*xhat[2]+A[0][3]*xhat[3]+B[0]*u+L[0][0]*(theta-xhat[0])+L[0][1]*(alpha-xhat[1]));
    xhat_new[1]=xhat[1]+0.001*(A[1][0]*xhat[0]+A[1][1]*xhat[1]+A[1][2]*xhat[2]+A[1][3]*xhat[3]+B[1]*u+L[1][0]*(theta-xhat[0])+L[1][1]*(alpha-xhat[1]));
    xhat_new[2]=xhat[2]+0.001*(A[2][0]*xhat[0]+A[2][1]*xhat[1]+A[2][2]*xhat[2]+A[2][3]*xhat[3]+B[2]*u+L[2][0]*(theta-xhat[0])+L[2][1]*(alpha-xhat[1]));
    xhat_new[3]=xhat[3]+0.001*(A[3][0]*xhat[0]+A[3][1]*xhat[1]+A[3][2]*xhat[2]+A[3][3]*xhat[3]+B[3]*u+L[3][0]*(theta-xhat[0])+L[3][1]*(alpha-xhat[1]));
    xhat[0]=xhat_new[0];
    xhat[1]=xhat_new[1];
    xhat[2]=xhat_new[2];
    xhat[3]=xhat_new[3];
    //Serial.print(A[2][1]);
    if (fabs(alpha)<=30*M_PI/180){
      
      motorVoltage = -((kgain[0]*theta)+(kgain[1]*alpha)+(kgain[2]*xhat[2])+(kgain[3]*xhat[3]));
      //motorVoltage = -((kgain[0]*theta)+(kgain[1]*alpha));
      //motorVoltage = -(5.12*theta-58*alpha);
      //u=-(kgain[0]*theta+kgain[1]*alpha);
    }  
    else{
       motorVoltage=0;
    }
