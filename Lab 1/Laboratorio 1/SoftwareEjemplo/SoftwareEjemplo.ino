...
const long sampleTime = 14200; //2470;  // set the sample time (the interval between SPI transactions) to 1000us = 1ms (90000 - 45000)
...
float theta = 0;
float R = 2*M_PI; //Señal de Referencia 
float R1 = M_PI/6;
float Kp = 0;
float Error = 0;
float ErrorOld = 0;
...
float UiOld = 0;
float UdOld = 0;
int tipoControl = 1; //Para seleccionar entre PD(0) y PV(1)
...
void loop() {
  ...
  if (currentMicros - previousMicros >= sampleTime) {
    ...
    if (currentMicros >= 0.5*CambioRef) {
      R = R1;
      LEDGreen = 999;}
        
    if (currentMicros >= 1.5*CambioRef) {
      R = 2*R1;
      LEDRed = 999;}
   
    if (currentMicros >= 2*CambioRef) {
      R = 3*R1;
      LEDBlue = 999;}

    if (currentMicros >= 3*CambioRef) {
      R = 4*R1;
      LEDGreen = 0;
      LEDRed = 0;
      LEDBlue = 0;}

    if (currentMicros >= 3.5*CambioRef) {
      R = 5*R1;
      LEDGreen = 0;
      LEDRed = 0;
      LEDBlue = 0;}

    if (currentMicros >= 4.5*CambioRef) {
      R = 0;
      LEDGreen = 0;
      LEDRed = 0;
      LEDBlue = 0;}
...
// Implementación del controlador
    ErrorOld = Error; // Se guarda el valor antiguo que tenga Error
    Error = R - theta; // Señal de error, cambia su valor
    UprimOld = Uprim; //Se guarda el valor U(n-1)


    if(tipoControl == 0){ //Controlador PD en funcionamiento
      Uprim = (0.2133 * Error);// - (0.06501 * ErrorOld) + (0.2417 * UprimOld)  ;
      Uprim = (0.5 * Error);

    }else{ //Controlador PV en funcionamiento
      Uprim = (0.19560332550094 * Error) - (0.002133333333333 * motorVelocity);
    ...
    if (motorVoltage > 10){motorVoltage=10;}
    if (motorVoltage < -10){motorVoltage=-10;}
...
void resetQUBEServo() {
 ...
 /* //Control Cascada
    //Uprim = 4*Error - 3.979836734693877*ErrorOld + UprimOld;
    Uprim = 4*Error;
    ErrorSec = Uprim - motorVelocity;
    motorVoltage = 0.008695652173913*ErrorSec - 0.008169653948536*ErrorSecOld + mVold;
    
    UprimOld = Uprim;
    ErrorSecOld = ErrorSec; */
